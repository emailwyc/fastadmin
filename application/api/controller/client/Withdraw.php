...
class Withdraw extends Api
{
    ...
    public function zd_allot($order_id, $uid)
    {
        ...
        if ($re) {
            $this->setAllot($data['allotdcid'],1);
            $this->setAllot($data['allotfcid'],2);
        } else {
            $this->setMessage($uid, 'shy_f');
    ...
    public function setAllot($uid,$type=0)
    {
        $info = Db::name('allot')->where(array('uid' => $uid,'type'=>$type))->find();
        if (empty($info)) {
            $data['uid'] = $uid;
            $data['num'] = 1;
            ...
            $data['num'] = $info['num'] + 1;
            $data['allot_time'] = time();
            Db::name('allot')->where(array('uid' => $uid,'type'=>$type))->update($data);
        }
    }
    ...
    public function getMin($type)
    {
        ...
        $groupinfo = Db::name('auth_group_access')->alias('aga')
            ->field("aga.uid,al.num,al.`type`")
            ->join("d_allot al", array("al.uid  = aga.uid","al.type=$type"),'left')
            ->where(array('aga.group_id'=>$grop_id))
            ->order("al.num asc")
            ->find();
        $sql = Db::name('auth_group_access')->getLastSql();
        Log::write(__FUNCTION__ . '：sql=' . $sql, 'error');
        $uid = $groupinfo['uid']?$groupinfo['uid']:0;
        return $uid;
    }
...
//            Db::name('sh_log')->insert($data);
    ...
    public function createOrder($params)
    {
        ...
        //分配资料审核员
///*        $field_group = "u.uid,count(o.id) as num,o.dcid";
////        $user_group = Db::name("auth_group_access")->alias("u")
////            ->field($field_group)
////            ->join("d_order o", "o.allotdcid = u.uid", "left")
////            ->where(array('u.group_id'=>11))
////            ->group("u.uid")
////            ->order('num asc')
////            ->find();
////
////        $data['allotdcid'] = $user_group['uid'];*/
...
//        $data['pay'] = $params['amount'] + $params['amount'] * $rate * ($params['cycle'] - 1);
    ...
    public function checkAuth2($uid, $params = array())
    {
        // TODO:  金额校验
        if (!empty($params['amount'])) {
        ...
        $authinfo = Db::name('user_authinfo')->where($where)->find();
        if (empty($authinfo)) {
            $this->error('淘宝认证失败', null, 9);
        }

        //通讯录是否认证
        $telebook = Db::name('user_telebook')->where(["uid" => $uid])->find();
        if (empty($telebook)) {
            $this->error('通讯录上传失败', null, 10); //表示还没有认证
        }
    }
