...
use app\api\controller\client\Withdraw;
...
//            $uid = 3;
    ...
    public function fqzforsh($ids)
    {
        ...
        if ($order) {
            ...
            if (empty($tbinfo)){
                ...
                $withdraw->decisionCheck($order['uid'],$order['id']);
                $tbinfo = Db::name("decision")->where(array('order_id' => $ids))->value("content");
    ...
    public function callrecords($ids){
        ...
        $data =  Db::name('call_records')->where(['uid'=>$uid])->find();

       if (!empty($data) && ($data['expired_time'] > time())){

           $call_records = json_decode(gzuncompress($data['content']),true);


       }else{
           $call_records = $this->create_callrecords($uid);
            $call_records = json_decode(gzuncompress($call_records),true);
    ...
    public function create_callrecords($uid){
...
//        var_dump($call_records);die();
        ...
        $data['content'] = gzcompress(json_encode($call_records));
        $data['uid'] = $uid;
        $data['create_time'] = time();
        $data['expired_time'] = time()+86400;
        $st =Db::name('call_records')->where(['uid'=>$uid])->find();

        if (!empty($st) && $st['expired_time']<time()){

            Db::name('call_records')->where(['uid'=>$uid])->update($data);
        }

        Db::name('call_records')->insert($data);
