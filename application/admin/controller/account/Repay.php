        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as &$value){
                ...
                if (in_array($value['status'],[2,4])){
                    $value['se'] = 1;//判断是否还款1 待处理 2  正常还款 3 逾期 4逾期还款 5 续期 6 展期  7逾期续期 8逾期展期 ',
                }
...
//            $this->success();
    ...
    public function settle($ids = null){
        ...
        if ($this->request->isPost()){
            ...
            if ($a==1){
                Log::write(__FUNCTION__.'订单:'.$ids.'结清成功','log');
                $this->success();
            }elseif ($a==-1){
                Log::write(__FUNCTION__.'订单:'.$ids.'结清失败','log');
                    $this->error('结清失败');
            }elseif ($a==-2){
                Log::write(__FUNCTION__.'订单:'.$ids.'结清更新失败','log');
        ...
        $order_id = Db::name('order_repay')->where(['id'=>$ids])->value('order_id');
        ...
        $data['fkje'] = number_format($data['fkje'],2,'.','');
