...
//                    $wheretype = "o.status=8";
                    $wheretype['o.status'] =['eq',8];
                    $wh = $wheretype;

                    break;
                case "reject":
                    $wheretype['o.status'] =['eq',16];
                    $wh = $wheretype;
                    break;
                default:
//                    $wheretype = "1=1";
                    $wheretype['o.status'] = array('NOT IN',[11,14,13,12,15]);
                    $wheretype['o.type'] = array('eq','1');
                    $wh['o.type'] = 2;
                    break;
            }
            $field
...
//                      ->where($where_neq)
            ...
            foreach ($list as $k=> &$v) {
                ...
                $mk_time = Db::name("order_repay")->where(["order_id" => $v['id']])->value("create_time");
                if ($mk_time){
                    $v['mk_time'] = date('Y-m-d H:i:s',$mk_time);
                }
...
//            }
            ...
            $fk_arr = [6,16];
            if (in_array($order['status'],$fk_arr) || $order['type']==2){
                $fk_s = true;
            }
...
//        $time_data['endtime'] = $repay_time + ($order['cycle']-1) * 24 * 60 * 60;
    ...
    public function approve($ids = null){
        ...
        if (!in_array($status,[12,14,15,16,17])){
            $this->error('订单不符合要求');
    ...
    public function k_fk($list){
        ...
        foreach ($list as $k=>&$v){
            ...
            if ($v['ut']==1){
                ...
                if (in_array($v['status'],$fk_1)){
                    $v['fk']=1;
                }
            }else{
                $fk_2 =[6,16,12,13];
                if (in_array($v['status'],$fk_2)){
                    $v['fk']=1;
                }
            }
        }
